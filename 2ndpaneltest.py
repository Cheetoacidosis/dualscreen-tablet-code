...
cd_pin = 23 # Command / Data pin
cs_pin = 24 # Chip select pin
busy_pin = 4 # Busy pin
...
def full_refresh(data):
    #Change panel settings back to normal mode that uses
    #full refresh, not lookup table
    send_cmd(0x00, [0x1F])

    #send old data, new data
    write_old_data(data)
    write_new_data(data)
    ...
    #Change back to fast refresh mode
...
screen_data_file = open("/dev/shm/display_data2.bin", "rb")
...
full_refresh_memory = open("/dev/shm/full_refresh2.bin", "rb")

screen_refresh_count = 0

#infinite loop while the program is running
while True:
    #read the screen info from the UI
    screen_data_file.seek(0)
    new_data = screen_data_file.read()
    #if the screen has changed,
    if len(new_data) == 48000 and new_data != old_data:
        #update the scree
    ...
    #if the full_refresh_activate.py script has been run,
    if full_refresh_memory.read() == b'\x01':
        #do a full refresh
        full_refresh(new_data)
