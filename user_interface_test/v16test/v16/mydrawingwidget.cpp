...
void mydrawingwidget::paintEvent(QPaintEvent *event) {
    ...
    for (const annotation_info &ann: annotations_list[pdf_page]) {
        ...
        QPen pen(Qt::black,ann.annotation_thickness*2);
        painter.setPen(pen);
        painter.drawPolyline(ann.annotation);
    }

    QPen pen(Qt::black,active_pen_mode*2);
    painter.setPen(pen);
...
//     annotations_list[pdf_page].clear();
//     current_annotation.clear();
//     update();
// }
...
void mydrawingwidget::saveToPdf(const QString &filePath, QPdfView *pdfView) {
    ...
    //check the point size of page
    QSizeF pdf_size = my_document->pagePointSize(this->pdf_page);

    //check point size of the pdf viewer
    QSize view_size = pdfView->size();

    //calculate how much the pdf is scaled in the pdf viewer
    double view_scale = std::min(view_size.width() / pdf_size.width(), view_size.height() / pdf_size.height());
    ...
    for (int i=0;i<my_document->pageCount()-1;i++) {

        //create an image that is a pixel-to-pixel copy of the pdf document
        QImage page_image = my_document->render(i, render_size);

        //create a painter that loads the pdf image
        QPainter pdf_painter(&page_image);
